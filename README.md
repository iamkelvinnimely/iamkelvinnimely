<div align="center">
  
```text
╔═══════════════════════════════════════════════════════════════════════════════════════════════════════╗
                                                                                                      
                 ██╗  ██╗███████╗██╗     ██╗   ██╗██╗███╗   ██╗                                         
                 ██║ ██╔╝██╔════╝██║     ██║   ██║██║████╗  ██║                                         
                 █████╔╝ █████╗  ██║     ██║   ██║██║██╔██╗ ██║                                         
                 ██╔═██╗ ██╔══╝  ██║     ╚██╗ ██╔╝██║██║╚██╗██║                                         
                 ██║  ██╗███████╗███████╗ ╚████╔╝ ██║██║ ╚████║                                         
                 ╚═╝  ╚═╝╚══════╝╚══════╝  ╚═══╝  ╚═╝╚═╝  ╚═══╝                                         
                                                                                                        
                   🚀 SOFTWARE DEVELOPER • EDTECH BUILDER 🚀                                           
                                  🇱🇷 LIBERIA                                                             
                                                                                                       
╚═══════════════════════════════════════════════════════════════════════════════════════════════════════╝
```

# 👋🏾 Hey, I'm Kelvin Nimely

### Software Developer • EdTech Builder • Technology Enthusiast

**I build software that solves real problems — especially problems close to home. 🇱🇷**

<br/>

<a href="https://iamkelvinnimely.vercel.app">
  <img src="https://img.shields.io/badge/🌐_Portfolio-iamkelvinnimely.vercel.app-111827?style=for-the-badge" alt="Portfolio"/>
</a>
&nbsp;
<a href="https://github.com/iamkelvinnimely">
  <img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>
&nbsp;
<img src="https://komarev.com/ghpvc/?username=kelvinnimely&label=Profile%20Views&style=flat-square" alt="Profile Views"/>

</div>

<br/>

---

## 👨🏾‍💻 About Me

I'm a **Software Developer and EdTech Trainer** from Liberia, passionate about using technology to solve practical problems and build products that can create real impact.

My journey is centered around three things:

* 💻 **Building software**
* 🎓 **Improving education through technology**
* 🌍 **Creating solutions for African problems**

I enjoy taking an idea from a conversation or a real-world problem and turning it into a working product.

```yaml
name: Kelvin Nimely
location: Liberia 🇱🇷
role:
  - Software Developer
  - EdTech Trainer
  - Technology Enthusiast

specialization:
  - Full-Stack Web Development
  - Backend Development
  - SaaS Products
  - EdTech
  - Cloud Infrastructure
  - Network Technology

currently_building:
  - EdCarry
  - PN Gateway
  - CodeDator
  - GM Range Farm Management System

philosophy: "Build useful things. Solve real problems."
```

---

## 🚀 What I Do

<table>
<tr>
<td width="50%" valign="top">

### 💻 Software Development

I build full-stack web applications and SaaS platforms using modern technologies.

* REST APIs
* Multi-tenant SaaS
* Authentication & authorization
* Dashboards & admin systems
* Real-time features
* Database architecture
* Cloud deployments

</td>

<td width="50%" valign="top">

### 🎓 EdTech

I work at the intersection of **technology and education**, helping schools and learners adopt digital tools.

* Educational platforms
* School management systems
* LMS platforms
* Teacher training
* Digital transformation
* Technology adoption

</td>
</tr>

<tr>
<td width="50%" valign="top">

### ☁️ Cloud & Infrastructure

Beyond writing application code, I enjoy understanding what happens when software reaches production.

* Docker
* Linux
* AWS
* VPS deployment
* PostgreSQL
* Redis
* Background workers
* Networking

</td>

<td width="50%" valign="top">

### 🌍 African Technology

I'm particularly interested in building technology for **Liberia and Africa**.

I believe many of our biggest opportunities are hidden inside everyday problems that haven't been properly solved yet.

</td>
</tr>
</table>

---

# 🛠️ Tech Stack

<div align="center">

### 💻 Languages

<p>
<img src="https://skillicons.dev/icons?i=python,js,ts,html,css" />
</p>

### 🌐 Frontend

<p>
<img src="https://skillicons.dev/icons?i=react,nextjs,tailwind" />
</p>

### ⚙️ Backend

<p>
<img src="https://skillicons.dev/icons?i=django,nodejs,express" />
</p>

### 🗄️ Databases & Infrastructure

<p>
<img src="https://skillicons.dev/icons?i=postgres,mongodb,redis,mysql" />
</p>

### ☁️ DevOps & Cloud

<p>
<img src="https://skillicons.dev/icons?i=docker,linux,aws,vercel,git,github" />
</p>

### 🎨 Design & Development Tools

<p>
<img src="https://skillicons.dev/icons?i=figma,vscode,postman" />
</p>

</div>

---

# 🚀 Things I've Built

## 🌐 PN Gateway

**Cloud management platform for WISPs and hotspot operators.**

PN Gateway helps network operators manage routers, customers, hotspot services and network operations from a centralized platform.

**Built with:**

`Django` `React/Vite` `PostgreSQL` `Redis` `Celery` `Docker` `AWS`

---

## 🎓 EdCarry

**School Management SaaS for schools.**

EdCarry is designed to help schools manage their academic and administrative operations digitally.

The platform focuses on bringing school management processes into one centralized system.

**Built with:**

`Django` `Django REST Framework` `NextJS` `PostgreSQL` `Redis` `Celery` `AWS` `S3`

---

## 📚 CodeDator

**A learning platform built for digital education.**

CodeDator is focused on creating a better environment for learning technology and programming online.

**Built with:**

`Next.js` `Node.js` `Express` `MongoDB`

---

## 🐄 GM Range Farm Management System

**Digital management system for livestock and poultry operations.**

Designed to help farms manage:

* 🐄 Livestock
* 🐔 Poultry
* 🌾 Feeding
* 💊 Animal health
* 📦 Inventory
* 💰 Finances
* 🥚 Egg production
* 📊 Farm analytics

The goal is simple:

> **Turn farm operations and records into useful data.**

---

# 🧠 Currently Exploring

<div align="center">

<img src="https://img.shields.io/badge/🤖_Artificial_Intelligence-Exploring-111827?style=for-the-badge"/>
<img src="https://img.shields.io/badge/☁️_Cloud_Architecture-Learning-111827?style=for-the-badge"/>
<img src="https://img.shields.io/badge/🏗️_SaaS_Architecture-Building-111827?style=for-the-badge"/>
<img src="https://img.shields.io/badge/🌍_African_Tech-Investing-111827?style=for-the-badge"/>

</div>

<br/>

I'm continuously learning, experimenting, and building around:

* 🤖 AI-powered applications
* 🧠 LLM integrations
* ☁️ Cloud architecture
* 🏗️ Scalable SaaS systems
* 🔐 Application security
* 🌐 Networking & infrastructure
* 📱 Mobile applications
* 💳 African payment integrations
---

# My Achievements
  
  <!-- GitHub Trophies -->
  <a href="https://github.com/ryo-ma/github-profile-trophy">
    <img src="https://github-trophies.vercel.app/?username=iamkelvinnimely&theme=discord&no-frame=true&no-bg=true&column=7&margin-w=10" alt="GitHub Trophies" width="100%"/>
  </a>

<br/>

---

# 📊 GitHub Analytics

<div align="center">

<!-- GitHub Stats + Custom Streak in ONE ROW -->
  <a href="https://github.com/red1-for-hek">
  <img height="195em" src="https://github-readme-stats-eight-theta.vercel.app/api?username=iamkelvinnimely&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=667eea&icon_color=764ba2&text_color=c9d1d9&include_all_commits=true&count_private=true" alt="GitHub Stats"/>
  </a>
  
  <br/>
  
  <!-- 📊 REAL-TIME LANGUAGE USAGE WITH PROGRESS BARS -->
  <a href="https://github.com/red1-for-hek">
    <img src="https://github-readme-stats-eight-theta.vercel.app/api/top-langs/?username=iamkelvinnimely&layout=normal&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=667eea&text_color=c9d1d9&langs_count=8&card_width=400" alt="Top Languages" />
  </a>

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=iamkelvinnimely&theme=tokyonight&hide_border=true" alt="GitHub Streak"/>

<br/>

<!-- Additional Stats Cards -->
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=iamkelvinnimely&theme=tokyonight" width="95%" alt="Profile Details"/>

</div>

---

# 💡 What I Believe

<div align="center">

### Technology doesn't have to be complicated to be impactful.

<br/>

**Find a problem.**

↓

**Understand it.**

↓

**Build a solution.**

↓

**Put it in the hands of people.**

↓

**Improve it.**

</div>

---

# 🌍 Building From Liberia

I'm proud to be building my career and products from **Liberia 🇱🇷**.

There is a huge opportunity for African developers to build products that aren't simply copies of solutions from other markets, but are designed around the realities of our own communities.

I'm interested in the intersection of:

**Technology × Education × Business × Africa**

And I'm just getting started.

---

# 📫 Let's Connect

<div align="center">

<a href="https://iamkelvinnimely.vercel.app">
<img src="https://img.shields.io/badge/Portfolio-111827?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Portfolio"/>
</a>

<a href="https://github.com/iamkelvinnimely">
<img src="https://img.shields.io/badge/GitHub-111827?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>

<a href="https://www.linkedin.com/in/iamkelvinnimely">
<img src="https://img.shields.io/badge/LinkedIn-111827?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>

<a href="https://www.facebook.com/iamkelvinnimely">
<img src="https://img.shields.io/badge/Facebook-111827?style=for-the-badge&logo=facebook&logoColor=white" alt="Facebook"/>
</a>

</div>

<br/>

---

<div align="center">

### 🚀 Keep Building. Keep Learning. Keep Solving.

<br/>

*"The best way to predict the future is to build it."*

<br/><br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=100&section=footer"/>

</div>
